...
export async function createOrganisationCollection(
  ...
  try {
    ...
    console.log('Organisation collection created with strict validation')

    const collection = db.collection(collectionName)

    await Promise.all([collection.createIndex({ orgId: 1 })])

    console.log('Organisation indexes created successfully')
  } finally {
    await client.close()
  }
}

export async function createRegistrationCollection(
  uri,
  dbName,
  collectionName
) {
  ...
  try {
    ...
    console.log('Registration collection created with strict validation')
    ...
    await Promise.all([
      ...
      collection.createIndex({ material: 1, wasteProcessingType: 1 }),
      collection.createIndex({ status: 1 }),
      collection.createIndex({ accreditationId: 1 }),
      collection.createIndex({
        'siteAddress.line1': 1,
        'siteAddress.postcode': 1,
        material: 1,
        wasteProcessingType: 1
      })
    ])

    console.log('Registration indexes created successfully')
  } finally {
    await client.close()
  }
}

export async function createAccreditationCollection(
  uri,
  dbName,
  collectionName
) {
  ...
  try {
    ...
    console.log('Accreditation collection created with strict validation')
    ...
    console.log('Accreditation indexes created successfully')
  } finally {
    await client.close()
  }
}
