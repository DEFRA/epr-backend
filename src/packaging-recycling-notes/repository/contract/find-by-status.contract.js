...
export const testFindByStatusBehaviour = (it) => {
  describe('findByStatus', () => {
    ...
    describe('organisation exclusion', () => {
      it('excludes PRNs belonging to excluded organisation ids', async () => {
        ...
        const testPrn = await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-TEST-${Date.now()}-${Math.random()}`,
            organisation: {
              id: testOrgId,
              name: 'Test Org',
              tradingName: 'Test'
        ...
        const realPrn = await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-REAL-${Date.now()}-${Math.random()}`,
            organisation: {
              id: realOrgId,
              name: 'Real Org',
              tradingName: 'Real'
      ...
      it('returns all items when excludeOrganisationIds is empty', async () => {
        const orgId = `org-all-${Date.now()}-${Math.random()}`

        const prn = await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-ALL-${Date.now()}-${Math.random()}`,
      ...
      it('derives hasMore and nextCursor from the filtered universe, not raw count', async () => {
        const sentinel = await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-S-${Date.now()}-${Math.random()}`
          })
        )
        ...
        const realPrn1 = await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-R1-${Date.now()}-${Math.random()}`,
            organisation: {
        ...
        const realPrn2 = await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-R2-${Date.now()}-${Math.random()}`,
            organisation: {
        ...
        await repository.create(
          buildCancelledPrn({
            prnNumber: `FBS-OE-T1-${Date.now()}-${Math.random()}`,
            organisation: {
              id: testOrgId,
              name: 'Test Org',
              tradingName: 'Test'
        ...
        expect(itemIds(result)).not.toContain(realPrn2.id)
      })
    })
  })
}
