...
describe('Summary logs integration', () => {
  ...
  describe('complete end-to-end submission flow with accreditation number', () => {
    const summaryLogId = 'summary-e2e-test'
    const fileId = 'file-e2e-123'
    const filename = 'e2e-test.xlsx'

    it('completes upload → validate → submit flow and returns accreditation number', async () => {
      // Set up test server with all components
      const summaryLogsRepositoryFactory = createInMemorySummaryLogsRepository()
      const mockLogger = {
        info: vi.fn(),
        error: vi.fn(),
        ...
        registrations: [
          {
            ...
            validTo: new Date('2025-12-31'),
            accreditation: {
              accreditationNumber: 'ACC-E2E-2025-999'
            }
          }
        ]
      })
        ...
        [fileId]: {
          ...
          data: {
            RECEIVED_LOADS_FOR_REPROCESSING: {
              ...
              rows: [['99999', '2025-01-15', '15 01 01', 1000, 100, 900]]
            }
          }
        }
      })
      ...
      // Step 1: Upload and mark as complete
      ...
      expect(validatedResponse.statusCode).toBe(200)
      const validatedPayload = JSON.parse(validatedResponse.payload)
      expect(validatedPayload.status).toBe(SUMMARY_LOG_STATUS.VALIDATED)

      // Step 3: Submit the validated summary log
      ...
      // Verify initial response shows SUBMITTING status
      const submittingPayload = JSON.parse(submitResponse.payload)
      expect(submittingPayload.status).toBe(SUMMARY_LOG_STATUS.SUBMITTING)

      // Step 4: Poll GET endpoint until status becomes SUBMITTED
      let attempts = 0
      const maxAttempts = 10
      let status = SUMMARY_LOG_STATUS.SUBMITTING
      let finalResponse
      ...
      while (
        ...
        finalResponse = await server.inject({
          method: 'GET',
          url: buildGetUrl(summaryLogId),
          headers: {
            Authorization: `Bearer ${validToken}`
        ...
        const payload = JSON.parse(finalResponse.payload)
        status = payload.status
        attempts++
      }

      // Step 5: Verify final state is SUBMITTED with accreditation number
      expect(finalResponse.statusCode).toBe(200)
      const finalPayload = JSON.parse(finalResponse.payload)
      expect(finalPayload.status).toBe(SUMMARY_LOG_STATUS.SUBMITTED)
      expect(finalPayload.accreditationNumber).toBe('ACC-E2E-2025-999')
    })
  })
})
