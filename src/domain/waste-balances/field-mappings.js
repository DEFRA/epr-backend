...
const REPROCESSOR_INPUT_RECEIVED_MAPPING = {
...
const REPROCESSOR_INPUT_SENT_ON_MAPPING = {
  [COMMON_FIELD.DISPATCH_DATE]: REPROCESSOR_INPUT_FIELD.DATE_SENT_ON,
  [COMMON_FIELD.SENT_ON_TONNAGE]: REPROCESSOR_INPUT_FIELD.SENT_ON_TONNAGE
}

const TEMPLATE_MAPPINGS = {
  [PROCESSING_TYPES.EXPORTER]: {
    [WASTE_RECORD_TYPE.EXPORTED]: EXPORTER_MAPPING
  },
  [PROCESSING_TYPES.REPROCESSOR_INPUT]: {
    [WASTE_RECORD_TYPE.RECEIVED]: REPROCESSOR_INPUT_RECEIVED_MAPPING,
    [WASTE_RECORD_TYPE.SENT_ON]: REPROCESSOR_INPUT_SENT_ON_MAPPING
  }
}
...
export const getFieldValue = (record, commonField) => {
  ...
  const typeMappings = TEMPLATE_MAPPINGS[processingType]
  if (!typeMappings) {
    throw new Error(
  ...
  const mapping = typeMappings[record.type]
  if (!mapping) {
    throw new Error(
  ...
  if (!sourceField) {
    throw new Error(
      `No mapping found for field: ${commonField} in record type: ${record.type} for processingType: ${processingType}`
    )
  }
