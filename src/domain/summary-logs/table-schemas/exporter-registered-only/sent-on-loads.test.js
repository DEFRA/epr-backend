...
describe('SENT_ON_LOADS (EXPORTER_REGISTERED_ONLY)', () => {
  ...
  describe('structure', () => {
    ...
    it('has sheetName set to Sent on (section 4)', () => {
      expect(schema.sheetName).toBe('Sent on (section 4)')
    })

    it('has rowTransformer set to transformSentOnLoadsRowExporterRegisteredOnly', () => {
      expect(schema.rowTransformer).toBe(
        transformSentOnLoadsRowExporterRegisteredOnly
      )
    })
