 ...
 * registration, so an operator with two sites counts twice, and a suspended
 * accreditation still owes its returns, so one that has not submitted is
 * outstanding. Every material and band is served for every month, at zero
 * where nothing is outstanding, so the publication's grid never loses a row.
 *
 ...
 * @property {{ months: Record<YearMonth, { figures: OutstandingByMaterial }> }} data
 */

/**
 * The outstanding returns for the given reporting months of a year, read from
 * the register and that year's monthly reports as they stand.
 *
 * @param {Object} params
 ...
 * @param {number} params.year - the reporting year the months fall in
...
export const buildOutstandingReturnsTable = async ({
  ...
  const outstanding = countOutstandingReturns({
    organisations,
    periodicReports,
    months
  })
  return {
    ...
    data: {
      months: recordOf(months, (month) => ({ figures: outstanding[month] }))
    }
  }
}
