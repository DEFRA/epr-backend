...
describe('closingForPrn', () => {
  ...
  it('ringfences available balance on creation, leaving total unchanged', () => {
    expect(closingForPrn(opening, STREAM_EVENT_KIND.PRN_CREATED, 200)).toEqual({
      amount: 1000,
      availableAmount: 600
    })
  })
  ...
  it('deducts total balance on issue, leaving available unchanged', () => {
    expect(closingForPrn(opening, STREAM_EVENT_KIND.PRN_ISSUED, 200)).toEqual({
      amount: 800,
      availableAmount: 800
    })
  })
  ...
  it('returns ringfenced tonnage to available on creation cancellation', () => {
    expect(
      closingForPrn(opening, STREAM_EVENT_KIND.PRN_CREATION_CANCELLED, 200)
    ).toEqual({ amount: 1000, availableAmount: 1000 })
  })

  it('returns tonnage to both balances when an issued PRN is cancelled', () => {
    expect(
      closingForPrn(opening, STREAM_EVENT_KIND.PRN_CANCELLED_AFTER_ISSUE, 200)
    ).toEqual({ amount: 1200, availableAmount: 1000 })
  })

  it('leaves the balance unchanged when a PRN is accepted', () => {
    expect(closingForPrn(opening, STREAM_EVENT_KIND.PRN_ACCEPTED, 200)).toEqual(
      opening
    )
  })

  it('leaves the balance unchanged when a PRN is rejected', () => {
    expect(closingForPrn(opening, STREAM_EVENT_KIND.PRN_REJECTED, 200)).toEqual(
      opening
    )
  })
  ...
  it('throws on an unrecognised event kind', () => {
    expect(() =>
      closingForPrn(opening, STREAM_EVENT_KIND.SUMMARY_LOG_SUBMITTED, 200)
