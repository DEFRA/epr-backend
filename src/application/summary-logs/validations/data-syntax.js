...
/**
 * Validates that all ROW_ID values are valid (numeric and >= 10000)
 *
 ...
 * and are FATAL errors that reject the entire spreadsheet.
 *
 * @param {Object} params
 * @param {string} params.tableName - Name of the table being validated
 * @param {Array<string|null>} params.headers - Array of header names from the table
 ...
 */
const validateRowIds = ({ tableName, headers, rows, location, issues }) => {
  const rowIdField = getRowIdField(tableName)
  const rowIdIndex = headers.indexOf(rowIdField)

  /* istanbul ignore next - Defensive: header validation catches missing ROW_ID first */
  if (rowIdIndex === -1) {
    return
  }

  for (const [rowIndex, row] of rows.entries()) {
    const rowIdValue = row[rowIdIndex]
    const isNumeric =
      typeof rowIdValue === 'number' && !Number.isNaN(rowIdValue)
    const isValidRange = isNumeric && rowIdValue >= MIN_ROW_ID

    if (!isNumeric || !isValidRange) {
      const cellLocation = location?.column
        ? {
            sheet: location.sheet,
            table: tableName,
            row: location.row + rowIndex + 1,
            column: offsetColumn(location.column, rowIdIndex),
            header: rowIdField
          }
        : { table: tableName, header: rowIdField }

      const message = !isNumeric
        ? `Invalid ROW_ID: value must be a number`
        : `Invalid ROW_ID: value must be at least ${MIN_ROW_ID}`
