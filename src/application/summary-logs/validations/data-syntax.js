...
/**
 * Validates rows against the fatal schema (e.g. ROW_ID validation)
 *
 ...
 * @param {import('joi').ObjectSchema} params.fatalSchema - Joi schema for fatal validations
 * @param {Object} params.location - Table location in spreadsheet
 * @param {ReturnType<typeof createValidationIssues>} params.issues - Validation issues collector
 */
const validateFatalFields = ({
  tableName,
  headers,
  rows,
  fatalSchema,
  ...
  for (const [rowIndex, originalRow] of rows.entries()) {
    ...
    const { error } = fatalSchema.validate(rowObject)
...
const validateTable = ({ tableName, tableData, schema, issues }) => {
  ...
  const { requiredHeaders, fatalSchema, rowSchema } = schema
