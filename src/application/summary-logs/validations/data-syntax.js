...
/**
 * Validates rows against the failure schema (e.g. ROW_ID validation)
 *
...
const validateRowForFailures = ({
  ...
  for (const [rowIndex, originalRow] of rows.entries()) {
    ...
    const { error } = failureSchema.validate(rowObject)
...
const validateRowForConcerns = ({
  ...
  return rows.map((originalRow, rowIndex) => {
    ...
    const { error } = concernSchema.validate(rowObject)
