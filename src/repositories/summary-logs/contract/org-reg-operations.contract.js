...
export const testOrgRegOperations = (it) => {
  describe('organisation/registration operations', () => {
    ...
    describe('hasNewerValidatedLog', () => {
      it('returns false when no logs exist for org/reg', async () => {
        const { organisationId, registrationId } = generateOrgReg()
        const result = await repository.hasNewerValidatedLog(
          organisationId,
          registrationId,
          'current-id'
        )
      ...
      it('returns false when only the current log exists', async () => {
        const currentId = `summary-${randomUUID()}`
        const { organisationId, registrationId } = generateOrgReg()
      ...
      it('returns true when another validated log exists', async () => {
        const currentId = `summary-${randomUUID()}`
      ...
      it('returns false when other logs are not validated', async () => {
        const currentId = `summary-${randomUUID()}`
      ...
      it('returns false for different org/reg', async () => {
        const currentId = `summary-${randomUUID()}`
        const otherId = `summary-${randomUUID()}`
        const { organisationId, registrationId } = generateOrgReg()
        const { organisationId: otherOrgId, registrationId: otherRegId } =
