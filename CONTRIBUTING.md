...
This project uses [Vitest](https://vitest.dev/) for testing and requires 100% code coverage.

##### Preferred Test Pattern

**For route tests and most application tests, use the `createTestServer()` helper:**
...
- Fast (no MongoDB startup - skips infrastructure when repositories are provided)
- Clean (explicit dependencies)
- Flexible (inject any test doubles)
- Includes logger mocks automatically

##### Test Patterns by Type

**Unit Tests** - Test pure functions with no infrastructure:
...
**Repository Contract Tests** - Ensure implementations match the interface:
...
// mongodb.test.js
import { testSummaryLogsRepositoryContract } from './port.contract.js'
import { createMongoSummaryLogsRepository } from './mongodb.js'
...
describe('MongoDB summary logs repository', () => {
  ...
  testSummaryLogsRepositoryContract(() =>
    createMongoSummaryLogsRepository(server.db)
  )
})
```

Contract tests run the same test suite against both in-memory and MongoDB implementations to ensure consistency.

**Integration Tests** - Test complete workflows with in-memory infrastructure:
...
const server = await createTestServer({
  ...
  workers: {
    summaryLogsValidator: { validate: mockValidator }
  },
  featureFlags: createInMemoryFeatureFlags({ summaryLogs: true })
})
```
...
**Vitest Fixtures** (extend pattern) - Only use when necessary:

- `dbTest` - Starts MongoDB (use only for repository contract tests)
- `s3Test` - Starts in-memory S3 server (use for S3 adapter tests)
- `serverTest` - Server without MongoDB (rarely needed, prefer `createTestServer()`)

**Helper Functions:**

- `createTestServer()` - **Preferred** for route and integration tests
- `createServer()` - Only use when you need specific control over server options

##### Anti-patterns to Avoid

❌ Don't use MongoDB for tests that don't need it:

```javascript
// BAD - starts MongoDB unnecessarily
import { testServerFixture as test } from '.vite/fixtures/test-server.js'

test('validates payload', async ({ testServer }) => {
  // This test only checks validation, doesn't need real DB
})
```
...
❌ Don't start MongoDB then mock it:

```javascript
// BAD - defeats the purpose of starting MongoDB
test('test', async ({ testServer }) => {
  vi.spyOn(testServer.db, 'collection').mockReturnValue(...)
})
```

✅ Use in-memory repositories that don't need mocking:

```javascript
// GOOD - test against real in-memory implementation
const repository = createInMemorySummaryLogsRepository()
const server = await createTestServer({
  repositories: { summaryLogsRepository: repository }
})
```

##### Legacy Patterns

Some tests use `testServerFixture` with MongoDB mocking - this is a legacy pattern for routes that haven't been refactored to use repositories yet. New tests should always use `createTestServer()` with in-memory repositories.
